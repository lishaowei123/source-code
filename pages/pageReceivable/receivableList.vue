<template>
	<view class="container">
		<template v-if="attrType == 1">
			<view class="baseView" v-if="attrSource.length">
				<view class="baseViewItem" v-for="(item, index) in attrSource" :key="index" @click="funcClickItem(item)">
					...
					<view class="person">回款日期: {{item.date || ""}}</view>
					<view class="person">方式: {{item.paytype && item.paytype.name || ""}}</view>
				</view>
			</view>
		...
		<template v-if="attrType == 2">
			<view class="baseView" v-if="attrSource.length">
				<view class="baseViewItem" v-for="(item, index) in attrSource" :key="index" @click="funcClickOrderItem(item)">
					<view class="">
						订单金额: ¥{{item.price || ""}}
					</view>
					<view class="person">客户: {{item.customer && item.customer.name || ""}}</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			let data =  JSON.parse(uni.getStorageSync('receivableList'))  
			let data1 = data[key1] || []
			this.attrSource = data1[key2] || []
			if(options.type) {

<template>
	<view class="container">
		...
		<text class="mainTitle" style="padding-top: 70rpx;">销售情况</text>
		<view class="topTotalBase" @click="funcPushList('return_census', 'hk_data', 1, '回款列表')">
			<view class="topPrice">
				<text>{{ attrSource.return_census && attrSource.return_census.count || 0 }}</text>
				<text class="explain">回款次数</text>
			</view>
			<view class="topTotal">
				<text>¥ {{ attrSource.return_census && attrSource.return_census.price || 0 }}</text>
				<text class="explain">回款金额</text>
			</view>
		</view>

		<text class="mainTitle">订单分析</text>	

		<view class="topPrice style1" @click="funcPushList('order_analyse', 'order_count_data', 2, '订单列表')">
			<text>{{ attrSource.order_analyse && attrSource.order_analyse.order_count || 0 }}</text>
			<text class="explain">销售订单数</text>
		</view>
		<view class="personItem" @click="funcPushList('order_analyse', 'order_complete_count_data', 2, '已完成回款订单')">
			<view class="personArrowItem">
				<text class="itemLabel">回款的客户</text>
				<view class="itemLevel">
					<text class="itemLevelText">{{attrSource.order_analyse && attrSource.order_analyse.order_complete_count || 0 }}</text>
					<image class="itemArrow" src="../../static/choose-Cade/choose-Cadex.png" mode=""></image>
				</view>
			</view>
		</view>
		<view class="personItem" @click="funcPushList('order_analyse', 'order_not_complete_count_data', 2, '未完成回款订单')">
			<view class="personArrowItem">
				<text class="itemLabel">未回款的客户</text>
				<view class="itemLevel">
					<text class="itemLevelText">{{attrSource.order_analyse && attrSource.order_analyse.order_not_complete_count || 0 }}</text>
					<image class="itemArrow" src="../../static/choose-Cade/choose-Cadex.png" mode=""></image>
				</view>
			</view>
		</view>
		
		<view class="topPrice style1" @click="funcPushList('order_price', 'order_total_price_data', 2, '已完成回款订单')">
			<text>{{ attrSource.return_census && attrSource.order_price.order_total_price || 0 }}</text>
			<text class="explain">销售订单金额</text>
		</view>
		<view class="personItem">
			<view class="personArrowItem">
				<text class="itemLabel">已回款的金额</text>
				<view class="itemLevel">
					<text class="itemLevelText">{{attrSource.order_price && attrSource.order_price.order_complete_price || 0 }}</text>
					<!-- <image class="itemArrow" src="../../static/choose-Cade/choose-Cadex.png" mode=""></image> -->
				</view>
			</view>
		</view>
		<view class="personItem">
			<view class="personArrowItem">
				<text class="itemLabel">未回款的金额</text>
				<view class="itemLevel">
					<text class="itemLevelText">{{attrSource.order_price && attrSource.order_price.order_not_complete_price || 0 }}</text>
					<!-- <image class="itemArrow" src="../../static/choose-Cade/choose-Cadex.png" mode=""></image> -->
				</view>
			</view>
		</view>
...
export default {
	...
	mounted() {
		...
		this.attrList[0].source.map(e => {
			console.log(e.value , moment().format('YYYY'));
	...
	destroyed() {
		uni.removeStorageSync('receivableList')
	},
	...
	methods: {
		 ...
		 * 跳转到列表
		 */
		funcPushList(key1, key2, type, title) {
			uni.navigateTo({
				url: `./receivableList?key1=${key1}&key2=${key2}&type=${type}&title=${title}`
			})
		},
		...
		funcGetOrderStatistical() {
			Api.getOrderPaymentStatistical({
				...this.attrSearchSource
			}).then(res => {
				this.attrSource = res.data || {};
				uni.setStorageSync('receivableList', JSON.stringify(res.data))
			});
		},

	...
	<view class="container">
		...
		<view class="" v-if="attrType == 1">
			<template v-if="attrSource.length">
				<text class="price grayPrice">¥ {{ attrPrice || 0 }}</text>
				<view class="cell" v-for="(item, index) in attrSource" :key="index" @click="funcClickIndex(item, index)">
					<view class="cellItem">
...
export default {
	...
	destroyed() {
		uni.removeStorageSync('expenseAnalysisDetail')
	},
	...
	methods: {
		 ...
		 * 跳转到详情
		 */
		funcClickIndex(item, index) {
			uni.navigateTo({
				url : `./expenseAnalysisList?index=${index}&title=${item.type || '报销'}`
			})
		},
		...
		funcGetOrderStatistical() {
			Api.getOrderSinglesStatistical({
				...
				if(this.attrType == 1) {
					this.attrSource = res.data && res.data.consume || [];
					this.attrPrice = res.data && res.data.consume_price || 0
				} else {
					this.attrSource = res.data && res.data.customers || [];
					this.attrPrice = res.data && res.data.customers_price || 0
				}
				uni.setStorageSync('expenseAnalysisDetail', JSON.stringify(this.attrSource))
			});
		},

<template>
	<view class="previewImage" :style="{ 'background-color': 'rgba(0,0,0,' + opacity + ')' }" v-if="show" @tap="close" @touchmove.stop.prevent>
		<swiper class="swiper" :current="index" @change="swiperChange" :disable-touch="swiper" :circular="circular">
			<swiper-item v-for="(img, i) in attrSourceImgs" :key="'swiper-item-'+i" :id="'swiper-item-'+i">
		...
		<view class="page" v-if="attrSourceImgs.length > 0">
			<text class="text">{{ index + 1 }} / {{ attrSourceImgs.length }}</text>
		</view>
		...
		<view class="desc" v-if="descs.length > 0 && descs.length == attrSourceImgs.length && descs[index].length > 0">{{ descs[index] }}</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		save(e) {
			var _this = this;
			var src = this.attrSourceImgs[this.index];
			//#ifdef MP-WEIXIN
			//提前向用户发起授权请求
			uni.authorize({
		...
		open(e) {
			...
			if (!isNaN(e)) {
				if(e>=this.attrSourceImgs.length){
					console.log('kxj-previewImage:打开参数无效');
				}else{
					this.index = e;
				}
			} else {
				var index = this.attrSourceImgs.indexOf(e);
				if(index===-1){
					this.attrSourceImgs = [e];
					this.index = 0;
					console.log('kxj-previewImage:未在图片地址数组中找到传入的图片，已为你自动打开单张预览模式')
				}else{
					this.index = this.attrSourceImgs.indexOf(e);
				}
			}
